-- Run like: dofile("i2c.lua")

-- From: https://github.com/docice/esp8266/blob/master/i2c_scanner.lua
...
gpio_pin= {4,2,0,1,3} -- this array maps internal IO references to GPIO numbers

-- user defined function: see if device responds with ACK to i2c start
function find_dev(i2c_id, dev_addr)
    i2c.start(i2c_id)
    local c=i2c.address(i2c_id, dev_addr, i2c.TRANSMITTER)
    i2c.stop(i2c_id)
    return c
end
